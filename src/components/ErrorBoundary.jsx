        ...
        <div className="min-h-screen flex items-center justify-center bg-neutral-50">
          <div className="text-center p-8 max-w-md">
            ...
            <div className="flex flex-col gap-1 text-center">
              ...
              {this.state?.error && (
                <div className="mt-4 text-left bg-white border border-neutral-200 rounded p-3 overflow-auto max-h-64">
                  <div className="text-sm font-mono text-neutral-800 whitespace-pre-wrap">
                    {String(this.state.error?.message || this.state.error)}
                  </div>
                  {this.state.errorInfo && (
                    <pre className="mt-2 text-xs text-neutral-600 whitespace-pre-wrap">{this.state.errorInfo?.componentStack}</pre>
                  )}
                </div>
              )}
            </div>
